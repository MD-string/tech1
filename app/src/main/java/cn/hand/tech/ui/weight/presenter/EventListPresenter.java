...
import cn.hand.tech.ui.weight.IEventView;
...
public class EventListPresenter {
	...
	public EventListPresenter(Context context, IEventView view) {
		mContext = context;
		mView = view;
...
//			showProgressDialog("正在检测设备状态...");
	...
	private void showProgressDialog(String text) {
		...
		mdialog = new MyCustomDialog(mContext, R.style.LoadDialog,text);
		mdialog.show();
		new Thread("cancle_progressDialog") {
			@Override
			public void run() {
	 ...
	 * 关闭进度对话框
	 */
	private void mcloseProgressDialog() {
		if (mdialog != null) {
			mdialog.dismiss();
			mdialog = null;
		}
